...
function buildTitle(rating, culture, topRisk, metrics) {
  if (rating === "准点清场") return "准点盖章型腿日";
  if (rating === "准点带债") return "带债准点型复工";
  if (rating === "加班救场") return "加班救火型复工";
  if (rating === "仓促复工") return "早退盖章型复工";
  if (rating === "红灯闪烁") return "红灯闪烁型复工";
  if (rating === "审计保留意见") return "审计留档型复工";
  if (culture === "事故连锁型" || metrics.remainingSeverity >= 6) return "事故串门型复工";
  if (topRisk.includes("历史") || culture === "历史遗留型") return "档案柜增殖型腿日";
  if (topRisk.includes("供氧")) return "账本发红型复工";
  if (topRisk.includes("泡沫") || culture === "泡沫叙事型") return "PPT 泡泡型复工";
...
function buildRegret(metrics) {
  ...
  if (failure.id === "overtime") return "加班救回了事故，但节奏文化变成会议加班型。";
  if (failure.type === "kpi") return regretForKpi(failure.id.replace("kpi:", ""));
  if (failure.type === "incident") return regretForIncident(failure.id.replace("incident:", ""));
...
function tipForIncident(id) {
  const lines = {
    ...
    file_pile: "腿部趴桌不是最大麻烦，真正的债在档案柜里。"
  };
